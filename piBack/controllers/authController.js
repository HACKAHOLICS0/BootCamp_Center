...
const forgotPasswordEmail = async (req, res) => {
  ...
  const contactFormSubmit = async (req, res) => {
    ...
    try {
        ...
        await newComplaint.save();  // Save complaint to DB

        // Call the sendEmail function
        await Email.sendContactEmail(emailData);
        
        // Send success response
        res.status(200).json({ message: 'Email sent and complaint saved successfully!' });
    } catch (error) {
